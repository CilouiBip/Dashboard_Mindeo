...
async function getExistingFields() {
  try {
    console.log(`Récupération des détails de la table ${TABLE_NAME}...`);
    ...
    console.log(`✅ Table trouvée! ID: ${table.id}, ${table.fields.length} champs existants.`);
    return table;
  } catch (error) {
...
async function updateTable() {
  // Récupérer la table existante avec son ID
  ...
  for (const field of missingFields) {
    const success = await addField(tableId, field);
    if (success) {
      addedCount++;
    }
  }
